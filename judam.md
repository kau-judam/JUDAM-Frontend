...
- `src/constants/data.ts`의 `FundingProject`에는 후원 화면 재사용을 위해 `pricePerBottle`, `bottleSize`, `alcoholContent`, `totalQuantity`, `estimatedDelivery`, `rewardItems`, `shippingFee`가 추가되어 있다. API 연결 대비용으로 `isMine`, `breweryUserId`, `ownerUserId`, `creatorId`, `breweryId`, `favoriteCount`도 선택 필드로 둔다.
...
- 펀딩 카드에서 백엔드 응답의 `isMine === true`면 최우선으로 `내 프로젝트` 배지를 표시한다. 그 다음 `breweryUserId`/`ownerUserId`와 로그인 사용자 `id`/`uid` 매칭, 기존 `creatorId`/`breweryId` 매칭을 사용한다. `isMine === false`이고 id 매칭이 없으면 양조장명 fallback을 막는다. `breweryName` 또는 `name`과 프로젝트 `brewery`의 양조장명 비교는 백엔드 소유권 필드가 없을 때만 마지막 fallback으로 사용한다. 특정 demo project id를 모든 양조장의 본인 프로젝트로 취급하는 예외는 사용하지 않는다.
...
- 펀딩 프로젝트 소유권 판단은 `src/features/funding/ownership.ts`의 `isFundingProjectOwnedByBrewery`를 기준으로 한다. 이 유틸은 백엔드 `isMine === true`, `breweryUserId`/`ownerUserId`와 로그인 사용자 `id`/`uid` 매칭, 기존 `creatorId`/`breweryId` 매칭, `isMine === false` 차단, 양조장명 fallback 순서로 처리한다. 펀딩 리스트, 펀딩 상세, 후원하기 직접 진입 방어, 양조장 대시보드, 양조일지 관리 화면에서 같은 기준을 사용한다.
...
[2026-05-27]: 펀딩 내 프로젝트 판정에 백엔드 소유권 필드 우선 적용.

[진행 내용]:
- 백엔드 코덱스 요청에 따라 프론트 로컬 추론보다 백엔드 응답의 `isMine` 또는 `breweryUserId === currentUserId`를 우선 사용하도록 정리했다.
- `src/constants/data.ts`의 `FundingProject`에 `isMine`, `breweryUserId`, `ownerUserId` 선택 필드를 추가했다.
- `src/features/funding/api.ts`의 펀딩 목록/상세 응답 타입에 `isMine`, `is_mine`, `breweryUserId`, `brewery_user_id`, `ownerUserId`, `owner_user_id`를 추가했다.
- `src/features/funding/apiMappers.ts`에서 목록/상세 응답의 소유권 필드를 `FundingProject`로 보존하도록 매핑했다.
- `src/features/funding/ownership.ts`의 판정 순서를 `isMine === true`, `breweryUserId`/`ownerUserId`와 로그인 사용자 id 매칭, `creatorId`/`breweryId` 매칭, `isMine === false` 차단, 양조장명 fallback 순서로 변경했다.
- `getFundingList`에 `mine` 옵션을 추가해 필요한 화면에서 `GET /api/fundings?mine=true`를 Authorization 필수 호출로 사용할 수 있게 했다. 일반 펀딩 탭은 기존처럼 `mine` 옵션 없이 `GET /api/fundings`를 사용한다.
- 토큰 없는 직접 조회 기준 `GET /api/fundings?page=0&size=10` 응답에는 `isMine`, `breweryUserId`, `ownerUserId`가 포함되며, 아리양조장 프로젝트는 `breweryUserId: 25`, `ownerUserId: 25`로 내려오는 것을 확인했다. 현재 에뮬레이터에서는 술샘양조장 프로젝트에 `내 프로젝트` 배지가 붙어 있어, 저장된 앱 세션/토큰은 아리 계정이 아니라 user id 18 계정으로 동작 중인 것으로 추정된다.
- 검증 결과 `npx.cmd tsc --noEmit`, `npx.cmd eslint app src --no-cache`, `git diff --check` 통과.
