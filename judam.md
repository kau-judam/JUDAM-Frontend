...
- 상세 화면의 후기 탭은 진행 중 펀딩에서는 진행 중 안내, 성사/종료된 펀딩에서는 후원자 후기 카드 또는 첫 후기 작성 유도 UI를 보여준다. 후기 상세/후기 작성 전용 라우트는 아직 앱에 없으므로 버튼을 누르면 프론트 mock 안내 모달을 보여준다.
...
- `second`는 탭 변경 시 URL query를 `intro`, `journal`, `qna`, `review`로 동기화한다. 현재 Expo 앱은 `/funding/[id]?tab=journal` 직접 진입만 양조일지 초기 탭으로 반영하고, Q&A/후기 query 동기화는 아직 하지 않았다.
...
- `second`에는 `/archive/review/:id`, `/funding/:id/review/:reviewId` 같은 후기 작성/상세 웹 라우트가 있지만, 현재 Expo 앱에는 해당 라우트가 없다. 그래서 후기 작성/상세는 프론트 mock 안내 모달까지만 연결했다.
...
- 성사/종료된 펀딩의 후기 탭은 후원자 후기 카드를 `second` 카드형 UI에 가깝게 보정했고, 후기 카드/후기 작성 버튼은 아직 라우트가 없어 프론트 mock 안내 모달로 연결했다.
...
[x] 프로젝트 상세 UI 및 양조장용 펀딩 프로젝트 약관/생성 UI 구현 - 상세/약관/생성 완료, 양조일지 등록 후 상세 탭 이동 및 단계별 더보기/좋아요/댓글 프론트 mock 상호작용 완료
