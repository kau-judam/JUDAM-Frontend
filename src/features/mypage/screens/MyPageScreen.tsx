...
type MyPageSulbtiResultWithAliases = MyPageSulbtiResult & {
  has_result?: boolean | null;
  bti_code?: string | null;
  sulbti_result_id?: number | string | null;
  character_name?: string | null;
  alcohol_label?: string | null;
};

function getSulbtiResultCode(result: MyPageSulbtiResult | null) {
  if (!result) return null;
  const source = result as MyPageSulbtiResultWithAliases;
  return resolveSulbtiCode(source.btiCode || source.bti_code || source.type);
}

function getSulbtiResultId(result: MyPageSulbtiResult | null) {
  if (!result) return null;
  const source = result as MyPageSulbtiResultWithAliases;
  return source.sulbtiResultId ?? source.sulbti_result_id ?? null;
}

function hasSulbtiResult(result: MyPageSulbtiResult | null) {
  if (!result) return false;
  const source = result as MyPageSulbtiResultWithAliases;
  return Boolean(source.hasResult || source.has_result || getSulbtiResultCode(result) || getSulbtiResultId(result));
}

function getSulbtiResultTitle(result: MyPageSulbtiResult | null, fallback = '') {
...
export default function MyPageScreen() {
  ...
  const [sulbtiResult, setSulbtiResult] = useState<MyPageSulbtiResult | null>(null);
  const [isSulbtiLoading, setIsSulbtiLoading] = useState(false);
  ...
  const refreshMyPageData = useCallback((mountedRef: { current: boolean }) => {
    ...
    setIsSulbtiLoading(true);
    getMyPageSulbti()
      .then((nextSulbti) => {
        if (!mountedRef.current) return;
        const completed = hasSulbtiResult(nextSulbti);
        setSulbtiResult(completed ? nextSulbti : null);
        const sulbtiCode = completed ? getSulbtiResultCode(nextSulbti) : null;
        if (sulbtiCode && sulbtiCode !== resolveSulbtiCode(user.sulbti)) {
          updateUser({ sulbti: sulbtiCode });
        }
      })
      .catch((error) => {
        if (!mountedRef.current) return;
        setSulbtiResult(null);
        if (isAuthExpiredError(error)) {
          void handleAuthExpired();
          return;
        }
      ...
      .finally(() => {
        if (mountedRef.current) setIsSulbtiLoading(false);
      });
  ...
  const sulbtiCode = getSulbtiResultCode(sulbtiResult);
  const sulbtiResultId = getSulbtiResultId(sulbtiResult);
  ...
  const openBtiResult = () => {
    ...
    router.push({
      ...
      params: {
        type: sulbtiCode,
        btiCode: sulbtiCode,
        sulbtiResultId: sulbtiResultId ? String(sulbtiResultId) : '',
      },
    } as any);
  };
  const startBtiTest = () => {
