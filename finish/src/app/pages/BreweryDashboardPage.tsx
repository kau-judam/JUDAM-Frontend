...
export function BreweryDashboardPage() {
  ...
  const hasNotifications = useMemo(() => {
    ...
    // 2. 펀딩이 80% 이상 도달
    const highProgressFundings = activeFundings.filter(funding => funding.progress >= 80);
    ...
    return popularRecipes.length > 0 || highProgressFundings.length > 0 || hasQnA || summaryStats.newNotifications > 0;
  }, []);
        ...
        <section className="px-5 py-5 bg-white">
          ...
          <div className="space-y-4">
            {filteredFundings.map((funding, index) => {
              const progressPercentage = Math.min(funding.progress, 100);
                ...
                >
                  <div className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 border border-gray-100 p-4">
                    <div className="flex gap-4">
                      ...
                      {/* 콘텐츠 영역 - 오른쪽 */}
                      <Link to={`/funding/${funding.id}`} className="flex-1 min-w-0">
                        <div>
                          {/* 상단: 양조장 이름 */}
