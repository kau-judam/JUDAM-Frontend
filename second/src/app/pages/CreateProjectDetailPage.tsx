...
일정 지연: 술이 충분히 익지 않았을 경우, 최상의 맛을 위해 출고가 최대 10일 정도 지연될 수 있으며 이 경우 커뮤니티를 통해 즉시 공지하겠습니다.`,
  ...
  const mockAddresses = [
    ...
    { zipCode: "48058", address: "부산광역시 금정구 금성동 123-45" },
  ];
  ...
  const isStartDatePast = () => {
    if (!fundingInfo.startDate) return false;
  ...
  const isDeliveryDateValid = () => {
    if (!fundingInfo.startDate || !fundingInfo.expectedDeliveryDate) return true;
    const startDate = new Date(fundingInfo.startDate);
  ...
  const confirmSubmit = () => {
    ...
    // 시작일과 종료일 계산
    const today = new Date();
    const startDate = fundingInfo.startDate
      ? new Date(fundingInfo.startDate)
      : today;
    const endDate = new Date(startDate);
    endDate.setDate(endDate.getDate() + (fundingInfo.duration || 30));

    const formatDate = (date: Date) => {
      return `${date.getFullYear()}. ${String(date.getMonth() + 1).padStart(2, '0')}. ${String(date.getDate()).padStart(2, '0')}`;
    };

    // 새 프로젝트 생성 - 모든 입력 정보 포함
    const newProject = {
      // 기본 정보
      title: basicInfo.title,
      brewery: creatorInfo.name || user?.breweryName || "양조장",
      breweryLogo: creatorInfo.profileImage ? undefined : "🍶",
      location: taxInfo.address || user?.breweryLocation || "위치 미정",
      category: basicInfo.category,
      shortDescription: basicInfo.summary,
      image: basicInfo.images[0] || "https://images.unsplash.com/photo-1582204964885-7bc2d62b6917?w=1080",
      images: basicInfo.images.length > 0 ? basicInfo.images : undefined,
      videoUrl: basicInfo.videoUrl || undefined,

      // 펀딩 정보
      goalAmount: parseInt(fundingInfo.goalAmount) || 5000000,
      currentAmount: 0,
      backers: 0,
      daysLeft: fundingInfo.duration || 30,
      status: "심사 중" as const,
      startDate: formatDate(startDate),
      endDate: formatDate(endDate),

      // 제품 상세
      mainIngredients: basicInfo.mainIngredient || undefined,
      subIngredients: basicInfo.subIngredient || undefined,
      alcoholContent: basicInfo.alcoholContent || productInfo.alcoholContent || undefined,
      bottleSize: productInfo.volume || undefined,
      volume: productInfo.volume || undefined,
      pricePerBottle: parseInt(fundingInfo.pricePerBottle) || undefined,
      targetQuantity: parseInt(fundingInfo.bottleQuantity) || undefined,
      estimatedDelivery: fundingInfo.expectedDeliveryDate || undefined,
      ...
      // 프로젝트 계획
      introduction: projectPlan.introduction || undefined,
      rewardDetails: projectPlan.rewardDetails || undefined,
      budget: projectPlan.budget ?
        projectPlan.budget.split('\n').filter(line => line.trim()).map((line, index) => {
          const match = line.match(/(.+?)\s*[:\-]\s*(\d+)/);
          return match ? { item: match[1].trim(), amount: parseInt(match[2]) } : { item: line, amount: 0 };
        }) : undefined,
      schedule: projectPlan.schedule ?
        projectPlan.schedule.split('\n').filter(line => line.trim()).map((line) => {
          const match = line.match(/(.+?)\s*[:\-]\s*(.+)/);
          return match ? { date: match[1].trim(), description: match[2].trim() } : { date: "", description: line };
        }) : undefined,
      team: projectPlan.team || undefined,

      // 양조장 정보
      breweryBio: creatorInfo.bio || undefined,
      breweryProfileImage: creatorInfo.profileImage || undefined,

      // 법적 고시 정보
      productType: productInfo.productType || "막걸리",
      ingredients: productInfo.ingredients.filter(ing => ing.ingredient && ing.origin).length > 0
        ? productInfo.ingredients.filter(ing => ing.ingredient && ing.origin)
        : undefined,
              ...
              {activeTab === "funding" && (
                <div className="space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    {isStartDatePast() && (
                      <p className="text-xs text-red-600 mb-1">⚠️ 펀딩 시작일은 오늘 이후여야 합니다.</p>
                    )}
                    <input
                      type="date"
                  ...
                  <div className="space-y-2">
                    ...
                    {!isDeliveryDateValid() && fundingInfo.expectedDeliveryDate && (
                      <p className="text-xs text-red-600 mb-1">⚠️ 배송 예정일은 펀딩 시작일보다 미래여야 합니다.</p>
                    )}
                    <input
                      type="date"
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          className="px-4 py-3 bg-gray-900 hover:bg-gray-800 text-white text-sm font-medium rounded-xl transition-colors whitespace-nowrap"
                        >
                          주소 검색
                        </button>
                      </div>
