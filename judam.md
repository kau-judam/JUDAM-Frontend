...
API 통신 현재 상태: 2026-05-10 기준 다른 작업자가 `[주담]` 레시피 영역 일부 API 연결을 시도한 상태다. `api.md`에 백엔드 명세와 연결 로그가 있으며, 현재 서버 base URL은 `http://43.202.24.223:3000`이다. 레시피 API client는 `src/features/recipe/api.ts`에 있고 `fetch` 기반으로 레시피 목록/상세/작성/관심/댓글/댓글 좋아요 일부를 호출한다. 펀딩 API client는 `src/features/funding/api.ts`이며, 현재 펀딩 생성 흐름의 약관 동의 저장, 임시저장 생성/수정, 기본정보, 목표 금액 및 일정, 법적 고시 정보, 맛지표, 프로젝트 계획, 창작자/정산/사업자 정보, 안내사항 저장 API가 연결되어 있다. 펀딩 조회 5차 API는 목록/상세/소개/양조일지 조회를 기존 `FundingProject` UI 모델에 merge하는 방식으로 연결되어 있고, 6차 API는 Q&A 목록/질문 등록/답글 등록/후기 목록 조회를 연결했다. 7차 API는 후원 옵션 조회, 주문 생성, 결제 요청, 주문 상세 조회가 연결되어 있다. 서버는 사용량 과금 대상이므로 불필요한 수동 호출을 피한다. 아직 인증/회원가입 정식 API는 연결되지 않았고, `AuthContext`에는 주담/펀딩 API 테스트용 임시 JWT 저장 로직이 남아 있다. 파일 업로드 API는 4차 필수 서류 상태 구조 수정 후 연결 대기 중이며, 후기 작성/수정 API는 아직 명세가 없어 기존 로컬 후기 작성 흐름을 유지한다. 현재 코드 기준 커뮤니티/마이페이지/술BTI는 여전히 기존 프론트 mock/로컬 상태 중심이다. OpenAI/Gemini/Anthropic SDK, Firebase/Supabase/Prisma/Express 등 AI/백엔드 내부 구현 패키지는 앱 코드에 없다.
...
- 양조장 mock 소유권은 `isFundingProjectOwnedByBrewery`에서 demo project id 1만 본인 프로젝트로 처리한다. 첫 번째 게시글만 양조장 로그인 시 예시 본인 프로젝트로 보인다.
...
[ ] 펀딩 API 연결 4차 PR: 필수 서류 업로드와 검증.
[x] 펀딩 API 연결 5차 PR: 펀딩 목록/상세/프로젝트 소개/양조일지 조회. 기존 UI 모델 보존을 위해 서버 응답을 `FundingProject`에 merge하는 방식으로 연결 완료.
[x] 펀딩 API 연결 6차 PR: Q&A 목록/질문 등록/답글 등록/후기 목록 조회 연결 완료. 후기 작성/수정 API는 명세가 없어 기존 로컬 흐름 유지.
[x] 펀딩 API 연결 7차 PR: 후원 옵션 조회, 주문 생성, 결제 요청, 주문 상세 조회 연결 완료. 결제 완료 callback/webhook 명세는 아직 없어 실제 결제 완료 확정은 백엔드 보강 필요.
...
- 후기 목록 조회는 API에 연결했지만, 후기 작성/수정 API 명세는 제공되지 않아 `FundingReviewWriteScreen`의 기존 로컬 후기 작성/수정 흐름은 유지했다.
