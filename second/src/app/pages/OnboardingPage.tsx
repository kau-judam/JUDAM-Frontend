...
function FeatureSlide({ slide }: { slide: (typeof SLIDES)[0] }) {
  ...
  return (
    <div className="relative h-full" role="article" aria-label={`슬라이드 ${slide.id + 1}: ${slide.title.replace('\n', ' ')}`}>
      {/* Background */}
      <ImageWithFallback
        src={slide.image}
        alt=""
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/35 to-black/15" aria-hidden="true" />

      {/* Subtle top vignette */}
      <div className="absolute inset-0 bg-gradient-to-b from-black/30 to-transparent h-40" aria-hidden="true" />
        ...
        <div className="pt-14">
          {slide.isBrandSlide ? (
            <motion.div
              ...
              role="banner"
            >
              <img src={onboardingLogo} alt="주담 로고" className="w-9 h-9 opacity-90" />
              <span
        ...
        <div className="pb-36">
          ...
          {slide.badge && BadgeIcon && (
            <motion.div
              ...
              role="status"
            >
              <BadgeIcon className="w-3 h-3 text-white" aria-hidden="true" />
              <span
