...
import { UserIcon, Building2, Wine, TrendingUp, Archive, FileCheck, FolderKanban, LayoutDashboard, ArrowLeft } from "lucide-react";
...
export function UserTypeSelectionPage() {
  ...
  const handleUserSelection = () => {
    ...
    // 일반 사용자로 시작 - 홈으로 바로 이동
    navigate("/home");
  };

  const handleBrewerySelection = () => {
    // 사용자 타입을 'brewery'로 업데이트
    if (user) {
      updateUser({ type: "brewery", isBreweryVerified: false });
    }
    // 양조장 인증 페이지로 이동
